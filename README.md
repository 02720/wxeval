...
- 日桶按"当日本地零点的lead"整日归桶，对非00Z起报的capture存在±12h的桶边界近似（00Z capture无此近似）
- 评分失败的capture不会被清理（防数据丢失），超过保留期+7天宽限后才强制删除
- state.json损坏时自动重置为空（所有capture会重新评分一次，结果CSV幂等不受影响）
